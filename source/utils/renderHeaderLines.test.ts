...
// Freeze Date.now for deterministic clock output
beforeEach(() => {
	// 2026-02-20 14:30:45 UTC
	vi.useFakeTimers();
	vi.setSystemTime(new Date('2026-02-20T14:30:45Z'));
});
...
function stripped(lines: [string, string, string]): [string, string, string] {
	return [stripAnsi(lines[0]), stripAnsi(lines[1]), stripAnsi(lines[2])];
}
...
describe('renderHeaderLines', () => {
	describe('invariants', () => {
		it('always returns exactly 3 lines', () => {
			for (const model of [fullModel, idleModel]) {
				for (const w of [40, 60, 80, 120]) {
					const lines = renderHeaderLines(model, w, false);
			...
			for (const model of [fullModel, idleModel]) {
				for (const hasColor of [true, false]) {
					for (const w of [40, 45, 60, 70, 80, 100, 120]) {
						const lines = renderHeaderLines(model, w, hasColor);
						for (let i = 0; i < 3; i++) {
		...
		it('right rail stable across status changes at same width', () => {
			...
			const lengths = statuses.map(status => {
				...
				const [line1] = stripped(renderHeaderLines(model, width, false));
				return line1!.length;
			});
	...
	describe('truncation order', () => {
		...
		it('ATHENA and status badge never dropped', () => {
			for (const w of [40, 50, 60, 80]) {
				const lines = stripped(renderHeaderLines(fullModel, w, false));
	...
	describe('content', () => {
		...
		it('shows progress only when present', () => {
			...
			const noProgress = stripped(renderHeaderLines(idleModel, 120, false));
		...
		it('shows ended when complete', () => {
			...
			const lines = stripped(renderHeaderLines(endedModel, 120, false));
			// formatClock uses local time, so just check the pattern
		...
		it('shows err/blk only when > 0, hidden when 0', () => {
			...
			const noErr = stripped(renderHeaderLines(idleModel, 120, false));
	...
	describe('clock format', () => {
		it('HH:MM:SS at width >= 70', () => {
			...
			// Clock should be HH:MM:SS format
			expect(lines[0]).toMatch(/\d{2}:\d{2}:\d{2}/);
		});

		it('HH:MM at width < 70', () => {
			const lines = stripped(renderHeaderLines(fullModel, 60, false));
			// Should have HH:MM but not HH:MM:SS
			// Check that there's a time pattern without seconds
			const timeMatches = lines[0]!.match(/\d{2}:\d{2}/g);
			expect(timeMatches).toBeTruthy();
			// Should not have HH:MM:SS
			expect(lines[0]).not.toMatch(/\d{2}:\d{2}:\d{2}/);
		});
	});

	describe('line 3 separator', () => {
		it('is dim dashes of width-1 length', () => {
			const lines = stripped(renderHeaderLines(fullModel, 80, false));
